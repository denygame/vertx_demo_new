package io.vertx.demo.service.discovery;
...
public class ServerServiceDiscovery extends AbstractVerticle {
	public static void main(String[] args) {
		Vertx vertx = Vertx.vertx();
		vertx.deployVerticle(ServerServiceDiscovery.class.getName());
	}
	...
	public void start() throws Exception {
		...
		Record record1 = new Record().setType("eventbus-service-proxy")
				.setLocation(new JsonObject().put("endpoint", "the-service-address")).setName("my-service")
		...
		discovery.getRecord(r -> true, ar -> {
			if (ar.succeeded()) {
				List<Record> results = (List<Record>) ar.result();
				System.out.println();
			} else {
				// lookup failed
			}
